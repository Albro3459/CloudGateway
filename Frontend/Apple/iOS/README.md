...
document. The UI displays per-region sync counts and can show/share the full
peer audit log via `ShareLink`, never written to disk or logs. Treat that log
as admin-only operational data: it can include user emails, client names,
client IDs, public keys, and tunnel IPs.
