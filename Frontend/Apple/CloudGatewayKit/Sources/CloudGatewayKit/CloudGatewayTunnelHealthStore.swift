...
public struct CloudGatewayTunnelHealthSnapshot: Codable, Equatable, Sendable {
    ...
    public init(tunnelIdentifier: String, health: CloudGatewayTunnelHealth, updatedAt: Date) {
...
public struct CloudGatewayTunnelHealthStore: Sendable {
    ...
    public func write(_ snapshot: CloudGatewayTunnelHealthSnapshot) throws {
        let url = try fileURL()
    ...
    public func read() throws -> CloudGatewayTunnelHealthSnapshot? {
        let url = try fileURL()
        guard FileManager.default.fileExists(atPath: url.path) else {
            return nil
        }
        let data = try Data(contentsOf: url)
        return try JSONDecoder.gatewayHealth.decode(CloudGatewayTunnelHealthSnapshot.self, from: data)
    }
