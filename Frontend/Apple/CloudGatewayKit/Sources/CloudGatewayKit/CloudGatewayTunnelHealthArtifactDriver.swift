...
private final class CloudGatewayTunnelHealthNotificationRegistrationToken: @unchecked Sendable {
...
private final class CloudGatewayTunnelHealthDeadlineSignal: @unchecked Sendable {
    private let lock = NSLock()
...
struct CloudGatewayTunnelHealthDesiredArtifacts: Equatable, Sendable {
    let snapshot: CloudGatewayTunnelHealthSnapshot?
    let notificationDesired: Bool
    let notificationRegistrationAllowed: Bool
    let notificationOperationID: CloudGatewayTunnelHealthOperationID?

    init(
        snapshot: CloudGatewayTunnelHealthSnapshot?,
        notificationDesired: Bool,
        notificationRegistrationAllowed: Bool,
        notificationOperationID: CloudGatewayTunnelHealthOperationID? = nil
    ) {
    ...
    static let empty = CloudGatewayTunnelHealthDesiredArtifacts(
        snapshot: nil,
...
actor CloudGatewayTunnelHealthArtifactDriver {
    ...
    private struct PendingOperation: Sendable {
        ...
        let notificationID: CloudGatewayTunnelHealthOperationID?
        let notificationKind: NotificationOperationKind?

        init(
            generation: UInt64,
            kind: PendingKind,
            notificationID: CloudGatewayTunnelHealthOperationID? = nil,
    ...
    private struct DeferredNotificationOperation: Sendable {
        ...
        let completion: @Sendable (CloudGatewayTunnelHealthEvent) async -> Void
    }
    ...
    private let persistence: any CloudGatewayTunnelHealthPersistenceAdapter
    private let notifications: any CloudGatewayTunnelHealthNotificationAdapter
    private let arbiter: CloudGatewayTunnelHealthEffectSubmissionArbiter
    private let intent: CloudGatewayTunnelHealthArtifactIntent
    private let notificationRepairDeadline: Duration
    private let now: @Sendable () -> Duration
    private nonisolated let deadlineSignal = CloudGatewayTunnelHealthDeadlineSignal()
    private var activeGeneration: UInt64?
    private var desired = CloudGatewayTunnelHealthDesiredArtifacts.empty
    ...
    init(
        ...
        gate: CloudGatewayTunnelHealthEffectSubmissionArbiter,
        intent: CloudGatewayTunnelHealthArtifactIntent = CloudGatewayTunnelHealthArtifactIntent(),
    ...
    func execute(
        ...
        generation: UInt64,
        completion: @escaping @Sendable (CloudGatewayTunnelHealthEvent) async -> Void
    ) {
        switch effect {
    ...
    private func startNotification(_ operation: DeferredNotificationOperation) {
        ...
        let physicalRegistration: CloudGatewayTunnelHealthNotificationRegistrationToken?
        ...
        let accepted = arbiter.submit(
            ...
            let callback: @Sendable (CloudGatewayTunnelNotificationResult) async -> Void = {
                [weak self] result in
                ticket.drain()
                let reportedResult: CloudGatewayTunnelNotificationResult
    ...
    private func cancelNotificationSubmission(
        key: PendingKey,
        physicalRegistration: CloudGatewayTunnelHealthNotificationRegistrationToken?
    ) {
        pending.removeValue(forKey: key)
        if let physicalRegistration {
            _ = finishPhysicalNotificationRegistration(physicalRegistration)
    ...
    private func startSnapshotRepairIfNeeded() {
        ...
        let callback: @Sendable (CloudGatewayTunnelEffectResult) async -> Void = {
            [weak self] result in
    ...
    private func satisfyDeferredNotification(
        with result: CloudGatewayTunnelNotificationResult
    ) async {
    ...
    private func beginPhysicalNotificationRegistration(
        generation: UInt64
    ) -> CloudGatewayTunnelHealthNotificationRegistrationToken {
        physicalNotificationRegistrationCounts[generation, default: 0] += 1
        return CloudGatewayTunnelHealthNotificationRegistrationToken(generation: generation)
    }

    private func finishPhysicalNotificationRegistration(
        _ registration: CloudGatewayTunnelHealthNotificationRegistrationToken
    ) -> Bool {
