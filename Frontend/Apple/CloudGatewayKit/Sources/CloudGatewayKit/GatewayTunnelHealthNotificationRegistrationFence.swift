...
public enum GatewayTunnelHealthNotificationAuthorizationResult: Sendable {
    case allowed
...
public final class GatewayTunnelHealthNotificationRegistrationFence: @unchecked Sendable {
    ...
    public func register(
        authorization: @escaping @Sendable (
            @escaping @Sendable (GatewayTunnelHealthNotificationAuthorizationResult) -> Void
        ) -> Void,
        add: @escaping @Sendable (
            @escaping @Sendable (GatewayTunnelNotificationResult) -> Void
        ) -> Void,
        remove: @escaping @Sendable () -> Void,
        completion: @escaping @Sendable (GatewayTunnelNotificationResult) async -> Void
    ) {
        let epoch = begin()
        let delivery = GatewayTunnelHealthNotificationResultDelivery(completion)
    ...
    private func staleResult(for epoch: Epoch) -> GatewayTunnelNotificationResult {
    ...
    private func resolveAddCompletion(
        ...
        lock.lock()
        let resolved: GatewayTunnelNotificationResult
        if current == epoch {
            resolved = result
        } else if current != nil {
            resolved = .unknown
...
private final class GatewayTunnelHealthNotificationResultDelivery: @unchecked Sendable {
    private let lock = NSLock()
    private var completion: (@Sendable (GatewayTunnelNotificationResult) async -> Void)?

    init(
        _ completion: @escaping @Sendable (GatewayTunnelNotificationResult) async -> Void
    ) {
        self.completion = completion
    }

    func deliver(_ result: GatewayTunnelNotificationResult) {
        lock.lock()
