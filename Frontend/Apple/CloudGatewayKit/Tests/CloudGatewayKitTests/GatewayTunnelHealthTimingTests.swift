...
private func timingMoment(_ monotonic: Int, wallOffset: TimeInterval) -> GatewayTunnelHealthMoment {
...
@Test func productionTunnelHealthTimingRetainsTheFrozenContract() {
    let timing = GatewayTunnelHealthTiming.production
...
@Test func evaluatorElapsedDeadlinesIgnoreWallClockJumps() {
    var evaluator = GatewayTunnelHealthEvaluator(
        startedAt: timingMoment(0, wallOffset: 0)
    )
    let neverHandshaked = GatewayTunnelRuntimeStats(
...
@Test func oneWayWindowUsesMonotonicTimeWhileHandshakeAgeUsesWallTime() {
    ...
    var staleEvaluator = GatewayTunnelHealthEvaluator(
        startedAt: timingMoment(0, wallOffset: 0)
    )
...
@Test func recoveryPathAndPersistenceDeadlinesUseMonotonicTime() {
    var recovery = GatewayTunnelRecoveryPolicy()
    _ = recovery.update(
        stats: nil,
        evidence: nil,
        path: .satisfied,
    ...
    var path = GatewayTunnelPathPolicy()
    ...
    var persistence = GatewayTunnelHealthPersistencePolicy()
...
@Test func snapshotFreshnessBoundsFutureDatesAndAcceptsCorrectingHeartbeat() {
    ...
    let corrected = GatewayTunnelHealthSnapshot(
        tunnelIdentifier: "client-1",
        health: .notPassingTraffic,
