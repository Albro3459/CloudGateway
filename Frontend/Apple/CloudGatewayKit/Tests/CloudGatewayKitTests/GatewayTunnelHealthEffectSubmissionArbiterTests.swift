...
@Test func submissionArbiterQueuedDeadlineUsesExactOnceStopBridge() async {
    ...
    let stopCompletion = GatewayTunnelStopCompletion {
...
private final class LockedHealthSubmissionTicket: @unchecked Sendable {
    private let lock = NSLock()
    private var storage: GatewayTunnelHealthEffectSubmissionTicket?

    func set(_ ticket: GatewayTunnelHealthEffectSubmissionTicket) {
        lock.lock()
        storage = ticket
        lock.unlock()
    }

    var value: GatewayTunnelHealthEffectSubmissionTicket? {
        lock.lock()
        let value = storage
        lock.unlock()
        return value
