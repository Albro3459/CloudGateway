...
private struct CoordinatorHarness {
    var session = CloudGatewayTunnelHealthSessionID(rawValue: 1)
    let wallStart = Date(timeIntervalSince1970: 1_700_000_000)
    var coordinator = CloudGatewayTunnelHealthCoordinator()
    var transition = CloudGatewayTunnelHealthTransition(effects: [], health: nil, nextWake: nil)

    mutating func start(
        at seconds: Int = 0,
        session: CloudGatewayTunnelHealthSessionID? = nil
    ...
    mutating func wake(at seconds: Int) throws -> CloudGatewayTunnelHealthTransition {
    ...
    mutating func stop(at seconds: Int) -> CloudGatewayTunnelHealthTransition {
        transition = coordinator.handle(.stop, at: moment(seconds))
        return transition
    }

    func moment(_ seconds: Int) -> CloudGatewayTunnelHealthMoment {
    ...
    func stats(
        ...
        rx: UInt64,
        tx: UInt64
    ) -> CloudGatewayTunnelRuntimeStats {
        CloudGatewayTunnelRuntimeStats(
...
private func runtimeReadID(
    in transition: CloudGatewayTunnelHealthTransition
...
private func recoveryEffect(
    in transition: CloudGatewayTunnelHealthTransition
) -> (CloudGatewayTunnelRecoveryRequest, CloudGatewayTunnelHealthOperationID)? {
    transition.effects.lazy.compactMap { effect in
        switch effect {
...
private func containsWithdrawal(_ transition: CloudGatewayTunnelHealthTransition) -> Bool {
...
private func driveToNotification(
    ...
    from start: Int,
    through end: Int
) throws -> CloudGatewayTunnelHealthOperationID {
    var notification: CloudGatewayTunnelHealthOperationID?
...
@Test func coordinatorRuntimeUnavailableUsesFullRecoveryLadder() throws {
    ...
    var binding: CloudGatewayTunnelHealthOperationID?
    var restart: CloudGatewayTunnelHealthOperationID?
    var confirmed = false
...
@Test func coordinatorIgnoresStaleWakeAndOperationCompletions() throws {
    ...
    let staleWake = CloudGatewayTunnelHealthWakeID(
        session: CloudGatewayTunnelHealthSessionID(rawValue: 99),
    ...
    let read = try #require(runtimeReadID(in: wake))
    let wrongID = CloudGatewayTunnelHealthOperationID(
        session: CloudGatewayTunnelHealthSessionID(rawValue: 2),
...
@Test func coordinatorMissingNotificationCallbackReconcilesBeforeRetrying() throws {
    ...
    _ = harness.start()
    _ = harness.path(satisfied: true, routeID: 1, at: 0)
    var registration: CloudGatewayTunnelHealthOperationID?
