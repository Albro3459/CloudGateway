...
@Test func stopDeadlineCompletesWhenAdapterCallbackNeverArrives() {
    ...
    let completion = CloudGatewayTunnelStopCompletion {
        completionCount += 1
    }
...
@Test func stopSubmissionSerializesDeadlineBehindClaimedSubmission() async {
    let queue = DispatchQueue(
        label: "CloudGatewayTunnelStopSubmissionTests",
    ...
    let submission = CloudGatewayTunnelStopSubmission(targetQueue: queue) {
        entered.signal()
        release.wait()
