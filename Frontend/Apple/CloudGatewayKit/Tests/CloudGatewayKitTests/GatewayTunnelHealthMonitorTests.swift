...
private final class ControllableRuntimeAdapter:
    ...
    @unchecked Sendable
{
    typealias ReadCompletion = @Sendable (GatewayTunnelRuntimeStats?) async -> Void
    typealias RecoveryCompletion = @Sendable (GatewayTunnelRecoveryResult) async -> Void

    let backendRestartCapability: GatewayTunnelBackendRestartCapability
    private let lock = NSLock()
    private var reads: [ReadCompletion] = []
    private var refreshes: [RecoveryCompletion] = []
    private var restarts: [RecoveryCompletion] = []

    init(capability: GatewayTunnelBackendRestartCapability = .supported) {
    ...
    func completeRead(_ index: Int, stats: GatewayTunnelRuntimeStats?) async throws {
...
private final class ControllablePersistenceAdapter:
    GatewayTunnelHealthPersistenceAdapter,
    @unchecked Sendable
{
    enum Kind: Equatable, Sendable {
        case write(GatewayTunnelHealthSnapshot)
        case clear
    }
    ...
    func complete(
        _ index: Int,
        result: GatewayTunnelEffectResult = .success
    ) async throws {
...
@Test func monitorIgnoresDuplicateRuntimeCompletion() async throws {
    ...
    let stats = GatewayTunnelRuntimeStats(
...
@Test func effectGateReservationDoesNotBlockSynchronousClose() async throws {
    ...
    let close = Task.detached {
        ...
        let generation = gate.closeCurrent()
        closed.signal()
        return generation
    }
...
@Test func artifactDriverRepairsOldWriteWithNewestSnapshot() async throws {
    ...
    let old = GatewayTunnelHealthSnapshot(
        tunnelIdentifier: "old",
        health: .unknown,
        updatedAt: Date(timeIntervalSince1970: 1)
    )
    let newest = GatewayTunnelHealthSnapshot(
        tunnelIdentifier: "new",
        health: .passingTraffic,
        updatedAt: Date(timeIntervalSince1970: 2)
    )
    let session1 = GatewayTunnelHealthSessionID(rawValue: 1)
    let id = GatewayTunnelHealthOperationID(session: session1, sequence: 1)
...
@Test func lostNotificationRepairDoesNotBlockNewestSnapshotRepair() async throws {
    ...
    let first = GatewayTunnelHealthSnapshot(
        tunnelIdentifier: "new",
        health: .unknown,
        updatedAt: Date(timeIntervalSince1970: 1)
    )
    ...
    let oldID = GatewayTunnelHealthOperationID(
        session: GatewayTunnelHealthSessionID(rawValue: 1),
        sequence: 1
    )
...
@Test func notificationRepairWaitsForCurrentRegistration() async throws {
    ...
    let currentID = GatewayTunnelHealthOperationID(
...
@Test func deferredNotificationIsCancelledWhenHealthRecovers() async throws {
    ...
    let session = GatewayTunnelHealthSessionID(rawValue: 1)
    let first = GatewayTunnelHealthOperationID(session: session, sequence: 1)
    let second = GatewayTunnelHealthOperationID(session: session, sequence: 2)
    gate.open(generation: 1)
    await driver.activate(
        generation: 1,
        desired: .init(
...
@Test func newerCoordinatorNotificationSupersedesMissingCallback() async throws {
    ...
    await driver.execute(.registerNotification(first), generation: 1) { _ in }
    for sequence in 2...5 {
        let next = GatewayTunnelHealthOperationID(
            session: session,
