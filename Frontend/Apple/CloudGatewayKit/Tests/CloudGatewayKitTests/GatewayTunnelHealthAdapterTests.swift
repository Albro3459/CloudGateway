...
private final class LockedAuthorizationCallback: @unchecked Sendable {
    typealias Callback = @Sendable (
        GatewayTunnelHealthNotificationAuthorizationResult
    ) -> Void
    ...
    func call(_ result: GatewayTunnelHealthNotificationAuthorizationResult) {
        lock.lock()
...
private final class LockedNotificationResultCallback: @unchecked Sendable {
    private let lock = NSLock()
    private var callback: (@Sendable (GatewayTunnelNotificationResult) -> Void)?

    func set(
        _ callback: @escaping @Sendable (GatewayTunnelNotificationResult) -> Void
    ) {
        lock.lock()
        self.callback = callback
        lock.unlock()
    ...
    func call(_ result: GatewayTunnelNotificationResult) {
        lock.lock()
        let callback = callback
        lock.unlock()
        callback?(result)
...
@Test func storeAdapterEnqueuesStalledWorkAndPreservesFIFO() async {
    ...
    let callbacks = AdapterCallbackRecorder<GatewayTunnelEffectResult>()
    let adapter = GatewayTunnelHealthStoreAdapter(
        write: { _ in
...
@Test func notificationRegistrationWithdrawalBlocksDelayedAuthorization() async {
    let fence = GatewayTunnelHealthNotificationRegistrationFence()
    let authorization = LockedAuthorizationCallback()
    let trace = LockedAdapterTrace<String>()
    let results = AdapterCallbackRecorder<GatewayTunnelNotificationResult>()
