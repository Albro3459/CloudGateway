...
* <b>DNS filtering</b>: AdGuard Home listens only on the WireGuard tunnel DNS IPs and forwards allowed VPN client queries upstream over DNS-over-TLS to Quad9, Mullvad, and LibreDNS (load balanced), so the cloud provider sees only encrypted DNS and never the domains clients resolve. Only the AdGuard DNS filter is enabled, and DNS query logs/statistics are disabled.
