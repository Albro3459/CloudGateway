...
# Unbound runs as a forward-only, DNSSEC-validating resolver between AdGuard
# Home and the upstream providers. AdGuard (filtering) sends queries to Unbound
# on 127.0.0.1:5335; Unbound forwards them over DNS-over-TLS to Quad9, Mullvad,
# and DNS.SB, so the cloud provider only ever sees encrypted DNS, never the
# domains clients resolve. Unbound validates DNSSEC locally against the root
# trust anchor, so answer integrity rests on the DNSSEC chain itself, not on
# trusting the upstream resolvers. It does NOT recurse (no root-hints), so it
# never talks plaintext to authoritative servers. Upstreams are pinned by IP
# because Unbound has no bootstrap resolver to look up their hostnames; the
# #name is the TLS certificate/SNI name used to authenticate each provider.
if command -v unbound-anchor >/dev/null 2>&1; then
  # unbound-anchor exits nonzero when it writes or updates the anchor, not only
  # on error, so a nonzero status here is not a failure. Judge success by
  # whether root.key actually exists afterward.
